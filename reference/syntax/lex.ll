...
but the Xyz parser and lexer are NOT properly reentrant because both
...
// FIXME: with bison 3.x, XYZ_STYPE appears in the generated
...
// way to tell flex to use XYZ_STYPE instead of YYSTYPE in the code
// it generates, or to tell bison to provide the definition of YYSTYPE
// in the generated oct-parse.h file.

#if defined (XYZ_STYPE_IS_DECLARED) && ! defined YYSTYPE
#  define YYSTYPE XYZ_STYPE
#endif

#define YY_NO_UNISTD_H 1
#define isatty xyz_isatty_wrapper
...
#define YY_EXTRA_TYPE xyz::base_lexer *
...
namespace xyz
{
  bool iskeyword (const std::string& s)
  {
    ...
    return (xyz_kw_hash::in_word_set (s.c_str (), s.length ()) != nullptr
...
<BLOCK_COMMENT_START>^{S}*{CCHAR}\}{S}*{NL} {
    ...
    if (curr_lexer->m_block_comment_nesting_level > 1)
      ...
      curr_lexer->finish_comment (xyz::comment_elt::block);
...
<LINE_COMMENT_START>^{S}*{CCHAR}\{{S}*{NL} {
    ...
    if (! curr_lexer->m_comment_text.empty ())
      curr_lexer->finish_comment (xyz::comment_elt::full_line);
...
<LINE_COMMENT_START>{S}*{CCHAR}{ANY_EXCEPT_NL}*{NL} {
    ...
    if (curr_lexer->m_filepos.column () == 1)
    ...
    else
      {
        ...
        curr_lexer->finish_comment (xyz::comment_elt::end_of_line);
    ...
    if (curr_lexer->start_state() != COMMAND_START)
      {
        ...
        curr_lexer->push_token (new xyz::token (DQ_STRING,
                                                   curr_lexer->m_string_text,
...
<DQ_STRING_START>(\.\.\.){S}*{NL} {
    ...
    /* FIXME: Remove support for '...' continuation in Xyz 9 */
    static const char *msg = "'...' continuations in double-quoted character strings were deprecated in version 7 and will not be allowed in a future version of Xyz; please use '\\' instead";

    std::string nm = curr_lexer->m_fcn_file_full_name;

    if (nm.empty ())
      warning_with_id ("Xyz:deprecated-syntax", "%s", msg);
    else
      warning_with_id ("Xyz:deprecated-syntax",
                       _("%s; near line %d of file '%s'"), msg,
...
<DQ_STRING_START>\\{S}+{NL} {
    ...
    /* FIXME: Remove support for WS after line continuation in Xyz 9 */
    static const char *msg = "whitespace after continuation markers in double-quoted character strings were deprecated in version 7 and will not be allowed in a future version of Xyz";
    ...
    if (curr_lexer->start_state() != COMMAND_START)
      {
        ...
        curr_lexer->push_token (new xyz::token (SQ_STRING,
                                                   curr_lexer->m_string_text,
                                                   curr_lexer->m_tok_beg,
                                                   curr_lexer->m_tok_end));
...
\\{S}*{CCHAR}{ANY_EXCEPT_NL}*{NL} {
    ...
    /* FIXME: Remove support for '\\' line continuation in Xyz 9 */
    static const char *msg = "using continuation marker \\ outside of double quoted strings was deprecated and will be removed in future, use ... instead";
        ...
        else
          {
            ...
            if (ident == "@")
            ...
            else
              {
                ...
                if (xyz::iskeyword (ident))
                  tok = new xyz::token (LEXICAL_ERROR,
                ...
                else
                  {
                    ...
                    tok = new xyz::token (FCN_HANDLE, ident,
                                             curr_lexer->m_tok_beg,
                                             curr_lexer->m_tok_end);
                  }
...
. {
    ...
    if (c == 1)
    ...
    else
      {
        ...
        buf << _("invalid character '")
            << xyz::undo_string_escape (static_cast<char> (c))
            << "' (ASCII " << c << ")";

        // Use current file position for error token.
        xyz::token *tok
          = new xyz::token (LEXICAL_ERROR, buf.str (),
                               curr_lexer->m_filepos, curr_lexer->m_filepos);
...
void *
xyz_alloc (yy_size_t size, yyscan_t)
{
  return std::malloc (size);
}

void *
xyz_realloc (void *ptr, yy_size_t size, yyscan_t)
{
  return std::realloc (ptr, size);
}

void
xyz_free (void *ptr, yyscan_t)
{
  std::free (ptr);
}
...
Return true if @var{name} is an Xyz keyword.
      ...
      for (int i = 0; i < TOTAL_KEYWORDS; i++)
        {
          ...
          // FIXME: The following check is duplicated in xyz::iskeyword.
...
#define XYZ_YYG                                                      \
  ...
  base_lexer::make_keyword_token (const std::string& s)
  {
    ...
    const xyz_kw *kw = xyz_kw_hash::in_word_set (s.c_str (), slen);
      ...
      case magic_line_kw:
        {
          ...
          xyz_value ov_value (static_cast<double> (l));
  ...
  if (unsigned_val)
    {
     switch (bytes)
       {
       case 1:
         return xyz_value (xyz_uint8 (long_int_val));

       case 2:
         return xyz_value (xyz_uint16 (long_int_val));

       case 4:
         return xyz_value (xyz_uint32 (long_int_val));

       case 8:
         return xyz_value (xyz_uint64 (long_int_val));
  ...
  else
    {
      ...
      switch (bytes)
        {
        case 1:
          return xyz_value (xyz_int8 (int8_t (long_int_val)));

        case 2:
          return xyz_value (xyz_int16 (int16_t (long_int_val)));

        case 4:
          return xyz_value (xyz_int32 (int32_t (long_int_val)));

        case 8:
        return xyz_value (xyz_int64 (int64_t (long_int_val)));
...
namespace xyz
{
  ...
  base_lexer::handle_number<10> (void)
  {
    ...
    XYZ_LOCAL_BUFFER (char, tmptxt, yylng + 1);
    char *rp = yytxt;
    char *p = &tmptxt[0];
    ...
    while ((ch = *rp++))
      {
        switch (ch)
          ...
          case 'i':
          case 'j':
            // Xyz does not provide imaginary integers.
            digits_only = false;
            imag = true;
            break;
        ...
        if (errno != ERANGE)
          {
            ...
            xyz_base_value *magic_int;
            if (long_int_val > std::numeric_limits<int64_t>::max ())
              magic_int = new xyz_magic_uint (xyz_uint64 (long_int_val));
            else
              magic_int = new xyz_magic_int (xyz_int64 (long_int_val));

            ov_value = xyz_value (magic_int);
          }
      }
  ...
  base_lexer::maybe_warn_separator_insert (char sep)
  {
    ...
    if (nm.empty ())
      warning_with_id ("Xyz:separator-insert",
                       _("potential auto-insertion of '%c' near line %d"),
                       sep, m_filepos.line ());
    else
      warning_with_id ("Xyz:separator-insert",
                       _("potential auto-insertion of '%c' near line %d of file %s"),
  ...
  base_lexer::warn_single_quote_string (void)
  {
    ...
    if (nm.empty ())
      warning_with_id ("Xyz:single-quote-string",
                       _("single quote delimited string near line %d"),
                       m_filepos.line ());
    else
      warning_with_id ("Xyz:single-quote-string",
                       _("single quote delimited string near line %d of file %s"),
  ...
  base_lexer::warn_language_extension (const std::string& msg)
  {
    ...
    if (nm.empty ())
      warning_with_id ("XYZ:language-extension",
                       _("XYZ language extension used: %s"),
                       msg.c_str ());
    else
      warning_with_id ("XYZ:language-extension",
                       _("XYZ language extension used: %s near line %d of file %s"),
